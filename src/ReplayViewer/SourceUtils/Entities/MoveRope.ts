...
export class MoveRope extends KeyframeRope {
    ...
    private generateMesh(): MeshHandle[] {
        ...
        const mesh: IMeshData = {
            attributes: [VertexAttribute.position, VertexAttribute.normal, VertexAttribute.uv, VertexAttribute.uv2],
            elements: [],
            vertices: [],
            indices: []
        };
        ...
        this.map.getLeafAt(mid, leaf => {
            const ambient = (this.material.properties as SplineRopeMaterial).ambient = new Array<Vector3>(6);
