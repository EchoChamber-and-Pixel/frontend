...
export class StudioModel extends RenderResource<StudioModel> {
    ...
    private static getOrCreateMatGroup(matGroups: IMeshData[], attribs: VertexAttribute[]): IMeshData {
        ...
        const newGroup = MeshManager.createEmpty(attribs);
    ...
    private static readonly sampleAmbientCube_samples = new Array<Vector3>(6);
    private static readonly sampleAmbientCube_temp = new Vector3();
    private static sampleAmbientCube(leaf: BspLeaf, pos: IVector3, normal: IVector3): number {
    ...
    createMeshHandles(bodyPartIndex: number, transform: Matrix4, lighting?: (number[][] | BspLeaf), albedoModulation?: number): MeshHandle[] {
        const bodyPart = this.info.bodyParts[bodyPartIndex];
        const handles: MeshHandle[] = [];

        const matGroups: IMeshData[] = [];
        ...
        const tempPos = new Vector4();
        const tempNormal = new Vector4();
        ...
        for (let model of bodyPart.models) {
            for (let mesh of model.meshes) {
                ...
                attribs.push(VertexAttribute.rgb);

                const dstGroup = StudioModel.getOrCreateMatGroup(matGroups, attribs);
                const newElem = MeshManager.copyElement(srcGroup, dstGroup, mesh.element);

                const posOffset = MeshManager.getAttributeOffset(attribs, VertexAttribute.position);
                const normalOffset = MeshManager.getAttributeOffset(attribs, VertexAttribute.normal);
                const rgbOffset = MeshManager.getAttributeOffset(attribs, VertexAttribute.rgb);
                const vertLength = MeshManager.getVertexLength(attribs);
        ...
        for (let matGroup of matGroups) {
            MeshManager.transform4F(matGroup, VertexAttribute.position, pos => pos.applyMatrix4(transform), 1);
            MeshManager.transform4F(matGroup, VertexAttribute.normal, norm => norm.applyMatrix4(transform), 0);
...
export class StudioModelPage extends ResourcePage<IStudioModelPage, IStudioModel> {
    ...
    onLoadValues(page: IStudioModelPage): void {
        ...
        this.matGroups = new Array<IMeshData>(page.materials.length);

        for (let i = 0, iEnd = page.materials.length; i < iEnd; ++i) {
            const matGroup = page.materials[i];
            this.matGroups[i] = MeshManager.decompress(matGroup.meshData);
...
export class VertexLightingPage extends ResourcePage<IVertexLightingPage, number[][]> {
    ...
    onLoadValues(page: IVertexLightingPage): void {
        ...
        for (let i = 0, iEnd = page.props.length; i < iEnd; ++i) {
            ...
            for (let j = 0, jEnd = srcProp.length; j < jEnd; ++j) {
                dstProp![j] = Utils.decompress(srcProp[j]) as number[];
            }
        }
    ...
    protected onGetValue(index: number): number[][] {
        return this.props[index] as number[][];
    }
}
