...
export abstract class ResourcePage<TPayload, TValue> {
    ...
    private toLoad: IPageRequest<TValue, ResourcePage<TPayload, TValue>>[] | null = [];
    ...
    load(index: number, callback: (payload: TValue, page: ResourcePage<TPayload, TValue>) => void): TValue | void {
        if (this.page != null) {
...
export abstract class PagedLoader<TPayload, TValue, TPage extends ResourcePage<TPayload, TValue>> implements ILoader {
    ...
    load(index: number, callback: (payload: TValue, page: TPage) => void): TValue | void {
        if (this.pages == null) {
        ...
        for (let i = 0, iEnd = this.pages.length; i < iEnd; ++i) {
            ...
            if (index >= page.first && index < page.first + page.count) {
                return page.load(index, callback as any);
            }
        }
    ...
    private getNextToLoad(): TPage | null {
        let bestScore = 0;
        let bestIndex = -1;
    ...
    update(requestQuota: number): number {
        while (this.active < requestQuota) {
            ...
            ++this.active;
            Http.getJson<TPayload>(next.url, page => {
                --this.active;
