...
export class MapViewer extends Game {
    mainCamera!: Camera;
    ...
    protected onInitialize(): void {
        ...
        this.mainCamera = new Camera(this, 75);
        ...
        window.onhashchange = (ev: HashChangeEvent) => this.hashChange();
    ...
    private readonly onHashChange_temp = new Vector3();
    ...
    protected onDeviceRotate(deltaAngles: Vector3): void {
    ...
    private readonly lookAngs = new Vector2();
    ...
    private updateCameraAngles(): void {
        ...
        this.lookQuat.setAxisAngle(Vector3.unitZ, this.lookAngs.x);
        this.tempQuat.setAxisAngle(Vector3.unitX, this.lookAngs.y + Math.PI * 0.5);
    ...
    private readonly onUpdateFrame_temp = new Vector3();
    ...
    protected onUpdateFrame(dt: number): void {
        ...
        if (this.showDebugPanel !== this.debugPanelVisible) {
            ...
            if (this.debugPanel != null) {
                if (this.showDebugPanel) this.debugPanel.style.display = "";
        ...
        if ((this.cameraMode & CameraMode.CanMove) !== 0 && this.isPointerLocked() && this.map.isReady()) {
            ...
            const moveSpeed = 512 * dt
                * (this.isKeyDown(Key.Shift) ? 4 : 1)
                * (this.isKeyDown(Key.Alt) ? 0.333 : 1);
