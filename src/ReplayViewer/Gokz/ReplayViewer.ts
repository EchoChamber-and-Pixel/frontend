import { ChangedEvent, KeyDisplay, OptionsMenu, ReplayControls, ReplayFile, RouteLine, TickData, Utils } from ".";
import { CameraMode, MapViewer } from "../SourceUtils";
import { Event } from ".";
import { Vector2 } from "../Facepunch";
import { Key, MouseButton } from "../Facepunch/WebGame";
 ...
 */
export class ReplayViewer extends MapViewer {
    /**
     ...
     * * `sender: Gokz.ReplayViewer` - This ReplayViewer
     */
    readonly cameraModeChanged = new ChangedEvent<CameraMode, CameraMode, ReplayViewer>(this);
    ...
    private spareTime = 0;
    private prevTick: number | undefined = undefined;
    ...
    constructor(container: HTMLElement) {
        ...
        this.cameraModeChanged.addListener(mode => {
            ...
            if (this.routeLine != null) {
                this.routeLine.visible = mode === CameraMode.FreeCam;
            }

            this.canLockPointer = mode === CameraMode.FreeCam;
    ...
    protected onMouseDown(button: MouseButton, screenPos: Vector2, target: EventTarget): boolean {
        this.ignoreMouseUp = target !== this.canvas;
    ...
    protected onMouseUp(button: MouseButton, screenPos: Vector2, target: EventTarget): boolean {
        const ignored = this.ignoreMouseUp || target !== this.canvas;
        ...
        if (button === MouseButton.Left && this.replay != null && this.map.isReady()) {
    ...
    protected onKeyDown(key: Key): boolean {
        switch (key) {
            case Key.X:
                this.cameraMode = this.cameraMode === CameraMode.FreeCam
                    ? CameraMode.Fixed : CameraMode.FreeCam;
