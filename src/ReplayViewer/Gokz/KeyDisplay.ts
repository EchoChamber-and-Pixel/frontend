import { Button, EntityFlag, TickData } from ".";
import { Vector3 } from "../Facepunch";
import { CameraMode } from "../SourceUtils";
...
export class KeyDisplay {
    ...
    constructor(viewer: ReplayViewer, container?: HTMLElement) {
        ...
        viewer.showKeyDisplayChanged.addListener(showKeyDisplay => {
            if (showKeyDisplay && viewer.cameraMode === CameraMode.Fixed) this.show();
            else this.hide();
        });

        viewer.cameraModeChanged.addListener(cameraMode => {
            if (viewer.showKeyDisplay && cameraMode === CameraMode.Fixed) this.show();
            else this.hide();
        });
