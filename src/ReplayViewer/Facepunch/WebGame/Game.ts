import { ICommandBufferParameterProvider, CommandBufferParameter, UniformType, ShaderManager, MeshManager, MaterialLoader, TextureLoader, ModelLoader, MouseButton, Key, DrawList, Camera, CommandBuffer } from ".";
import { ILoader, Vector2 } from "..";
...
export class Game implements ICommandBufferParameterProvider {
    ...
    constructor(container: HTMLElement) {
        ...
        this.materialLoader = this.addLoader(new MaterialLoader(this)) as MaterialLoader;
        this.textureLoader = this.addLoader(new TextureLoader(this.context)) as TextureLoader;
        this.modelLoader = this.addLoader(new ModelLoader(this)) as ModelLoader;
        ...
        window.addEventListener("mousewheel", evnt => {
            const handled = this.onMouseScroll((evnt as any).wheelDelta / 400);
            if (handled) evnt.preventDefault();
            return handled;
        });
