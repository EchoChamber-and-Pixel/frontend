import { RenderTexture, TextureDataType, TextureFormat, TextureTarget } from ".";

export class FrameBuffer {
    private context: WebGLRenderingContext;
    private frameBuffer: WebGLFramebuffer | undefined;

    private ownsFrameTexture: boolean | undefined;
    private ownsDepthTexture: boolean | undefined = false;

    private frameTexture: RenderTexture | undefined;
    private depthTexture: RenderTexture | undefined;
    ...
    constructor(glOrTex: WebGLRenderingContext | RenderTexture, width?: number, height?: number) {
        ...
        if (width !== undefined) {
            ...
            this.frameTexture = new RenderTexture(gl,
                ...
                TextureDataType.Uint8, width, height as number);
        } else {
    ...
    addDepthAttachment(existing?: RenderTexture): void {
        ...
        if (existing == null && this.frameTexture !== undefined) {
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.frameBuffer as WebGLFramebuffer);
        gl.framebufferTexture2D(gl.FRAMEBUFFER,
            gl.DEPTH_ATTACHMENT,
            gl.TEXTURE_2D,
            (this.depthTexture as RenderTexture).getHandle(),
            0);

        this.unbindAndCheckState();
    }

    getColorTexture(): RenderTexture { return this.frameTexture as RenderTexture; }
    getDepthTexture(): RenderTexture { return this.depthTexture as RenderTexture; }
    ...
    resize(width: number, height: number): void {
        if (this.ownsFrameTexture && this.frameTexture) this.frameTexture.resize(width, height);
