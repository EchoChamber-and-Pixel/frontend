import { Vector3, Quaternion, Matrix4, IVector3, Box3, Euler, AxisOrder, Vector4 } from "..";
...
export class Entity {
    ...
    setPosition(valueOrX: IVector3 | number, y?: number, z?: number): void
    {
        if (y !== undefined) {
            ...
            this.position.set(x, y, z as number);
        } else {
    ...
    setAngles(valueOrPitch: IVector3 | number, yaw?: number, roll?: number): void {
        ...
        Entity.tempEuler.x = roll as number;
        Entity.tempEuler.y = pitch;
        Entity.tempEuler.z = yaw as number;
