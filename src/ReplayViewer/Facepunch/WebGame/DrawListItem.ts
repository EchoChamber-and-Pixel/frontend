import { DrawList, MeshHandle, Entity } from ".";
import { Matrix4 } from "..";
...
export class DrawListItem implements IDrawListItem {
    ...
    private meshHandles!: MeshHandle[] | null;
    ...
    addMeshHandles(handles: MeshHandle[]): void {
        ...
        for (let i = 0, iEnd = handles.length; i < iEnd; ++i) {
            const handle = handles[i].clone((!this.isStatic && this.entity != null ? this.entity.getMatrix() : null) as Matrix4);
    ...
    getMeshHandles(): MeshHandle[] {
        return this.meshHandles as MeshHandle[];
    }
}